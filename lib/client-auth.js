...
function loadWorkspaceConfig(raw = process.env.CLIENT_AUTH_CONFIG) {
  if (!raw) throw new AuthConfigurationError('CLIENT_AUTH_CONFIG is not configured.');
  ...
  for (const [key, credential] of Object.entries(parsed)) {
    ...
    if (!workspace || !credential || typeof credential !== 'object') {
    ...
    if (salt.length < 16 || hash.length !== HASH_LENGTH) {
      throw new AuthConfigurationError(`CLIENT_AUTH_CONFIG contains invalid credentials for ${workspace}.`);
    }

    workspaces.set(workspace, { salt, hash });
  }

  if (workspaces.size === 0) {
    throw new AuthConfigurationError('CLIENT_AUTH_CONFIG must contain at least one workspace.');
  }

  return workspaces;
}

async function verifyWorkspacePassword(workspaceValue, passwordValue, rawConfig) {
  const workspace = normalizeWorkspace(workspaceValue);
  const password = typeof passwordValue === 'string' ? passwordValue : '';
  const workspaces = loadWorkspaceConfig(rawConfig);
...
function safeNextPath(nextValue, workspaceValue) {
  const workspace = normalizeWorkspace(workspaceValue);
  const fallback = workspace ? `/clients/${workspace}/` : '/client-login';
